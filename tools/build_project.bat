...
:: Get absolute path to root to avoid relative path issues with spaces
pushd "%~dp0.."
set "ROOT=%CD%"
popd
...
gcc -std=c11 "%SHOWCASE_DIR%\main.c" "%SHOWCASE_DIR%\ui_gen.c" "%ROOT%\src\webcee.c" -I "%ROOT%\include" -o "%SHOWCASE_DIR%\showcase.exe" -lws2_32 -lpsapi
if %ERRORLEVEL% NEQ 0 (
