...
static void wce_url_decode(char* dst, const char* src) {
    char a, b;
    while (*src) {
        if ((*src == '%') &&
            ((a = src[1]) && (b = src[2])) &&
...
static char* wce_get_query_param(const char* query, const char* key) {
    if (!query || !key) return NULL;
    char search[64];
    snprintf(search, sizeof(search), "%s=", key);
    char* start = strstr(query, search);
    if (!start) return NULL;
    
    // Ensure we matched the key at the start or after an ampersand
    if (start != query && *(start - 1) != '&' && *(start - 1) != '?') {
        // False match (e.g. searching for "id" in "user_id=...")
        // Try searching again from next char
        return wce_get_query_param(start + 1, key);
    }
...
void process_request(int client_idx) {
	...
	if (strncmp(path, "/api/list", 9) == 0 && strcmp(method, "GET") == 0) {
		char* q = strchr(path, '?');
		if (q) {
            char* list_name = wce_get_query_param(q, "name");
			if (list_name) {
	...
	if (strncmp(path, "/api/update", 11) == 0 && strcmp(method, "POST") == 0) {
		char* q = strchr(path, '?');
		if (q) {
            char* key = wce_get_query_param(q, "key");
            char* val = wce_get_query_param(q, "val");
			if (key && val) {
	...
	if (strncmp(path, "/api/trigger", 12) == 0 && strcmp(method, "POST") == 0) {
		char* q = strchr(path, '?');
		if (q) {
            char* event_name = wce_get_query_param(q, "event");
			if (event_name) {
                char* arg = wce_get_query_param(q, "arg");
				wce_dispatch_event(event_name, arg ? arg : "");
				send_response(c->fd, "200 OK", "text/plain", "OK", 2);
                free(event_name);
                if (arg) free(arg);
				return;
